...
class Divergence(ABC):
    ...
    def fit(self, p_samples, q_samples, n_fit_epochs: int, p_eval, q_eval, n=0, cfg=None):
        ...
        # Note: erased this part, to avoid output clutter
        '''
        if isinstance(self._model, MLPClassifier):
            loss_path = os.path.join(Path(sys.argv[0]).resolve().parent.parent, 'mlp_discriminative_losses')
            ...
            fig_path = os.path.join(loss_path, f'loss_{current_time}_{self.dataset_name}.')
            plt.savefig(fig_path + 'png')
            tikzplotlib.save(fig_path + 'tex')
            plt.show()
            plt.close()
            plt.plot(self._model.validation_scores_)
            plt.title('Validation accuracy')
            plt.xlabel('Epochs')
            plt.ylabel('ACC')
            fig_path = os.path.join(loss_path, f'acc_{current_time}_{self.dataset_name}.')
            plt.savefig(fig_path + 'png')
            tikzplotlib.save(fig_path + 'tex')
            plt.show()
            plt.close()
            predictions = self._model.predict(dl_eval.dataset.X)
            predictions_train = self._model.predict(dl.dataset.X)
        '''
        # Evaluate mlp - validation set
