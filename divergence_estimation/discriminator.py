...
def plot_losses(losses, losses_eval, dataloader, dl_eval, n, name, seed=0, learning_rates=None):
    ...
    # Note: we do not plot to avoid clutter, but uncomment to see the plots
    '''
    if seed == 0:
        fig_name = f"../results/losses/{name}_{current_time}_discriminator_loss_M={dataloader.dataset.X.shape[0] / 2}_L={dl_eval.dataset.X.shape[0] / 2}_N={n}."
        fig.savefig(fig_name+'png')
        tikzplotlib.save(fig_name+'tex')
    else:
        fig_name = f"../results/losses/{name}_{current_time}_discriminator_loss_M={dataloader.dataset.X.shape[0] / 2}_L={dl_eval.dataset.X.shape[0] / 2}_N={n}_seed={seed}."
        fig.savefig(fig_name+'png')
        tikzplotlib.save(fig_name+'tex')
    '''
    return ax
...
class Discriminator(nn.Module):
    ...
    def train_loop(self, dataloader: DataLoader, n_epoch: int, optimizer=None, dl_eval=None, n=0, seed=0, cfg=None):
        ...
        for epoch in (range(n_epoch)):
            ...
            # print(f"Epoch [{epoch+1}/{n_epoch}]: {avg_loss=}")
            # print(f"Epoch [{epoch+1}/{n_epoch}]: {avg_loss_i=}")
            # For early stopping
            if dl_eval is not None:
                self.eval()
                with torch.no_grad():
                    ...
                    # print(f"Validation: Epoch [{epoch + 1}/{n_epoch}]: {avg_loss_eval=}")
